...
while True:
    try:
        I_numbers = input().strip().split(' ')[1:]
        R_numbers = input().strip().split(' ')[1:]
        R_numbers = sorted(set(map(int, R_numbers)))
        result = ''
        total = 0
        for R in R_numbers:
            R_count = 0
            R_result = ''
            for index, I in enumerate(I_numbers):
                if str(R) in I:
                    R_count += 1
                    total += 2
                    R_result += str(index) + ' ' + str(I) + ' '
            if R_count != 0:
                total += 2
                result += str(R)+' '+str(R_count) + ' ' + R_result
